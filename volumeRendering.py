...
def getRays(uv, focal, c2w):
    ...
    focal is a focal length for each uv coordinate - CURRENTLY FOCAL IS JUST 1 VALUE
...
def renderRays(rayOrigins, rayDirs, sceneDensityFunc, sampleCount=6, near=0, far=8, randSamples=True):
    ...
    sceneDensities, sceneSourceColours = sceneDensityFunc(samplePoints)
...
def renderScene(sceneDensityFunc, W, H, focal, c2w, sampleCount=6, near=0, far=8):
    ...
    return renderRays(rayOrigins, rayDirs, sceneDensityFunc, sampleCount, near, far, True)
